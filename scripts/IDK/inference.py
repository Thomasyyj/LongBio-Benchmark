...
from scripts.data_loader import LongContextDataSetFactory
from scripts.models import ModelsFactory
from scripts.utils import parse_config_from_data_path, save_json, load_json, split_list, custom_collate_fn
...
if __name__ == "__main__":
    ...
    # only for idk task
    args.data_path = args.data_path.replace('IDK', 'standard')

    # Load ori results
    ori_results_file_path = os.path.join(args.output_dir.replace('IDK', 'standard'), 'inference_results.json')
    if not os.path.isfile(ori_results_file_path):
        ori_results_file_path = os.path.join(args.output_dir.replace('IDK', 'standard')+"_2shot", 'inference_results.json')
    assert os.path.isfile(ori_results_file_path), f"we need the results for standard retrieval: {ori_results_file_path}"
    with open(ori_results_file_path, 'r') as f:
    ...
    data_configs['skill'] = "IDK" # only for idk task
    ...
    ori_results = sorted(ori_results, key=lambda r: r['datapoint']['context_idx'])
    evaluator = Evaluator(args, dataset, data_configs, ori_results=ori_results)
